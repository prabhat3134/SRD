		...
		REAL :: av=0.0, std=sqrt(kbt/m), g=1e-5, dt_c=1.0, start, finish, seed
		REAL (kind=dp1) :: alpha,vs = 1.0
		INTEGER :: tmax=5000, head(Ly,Lx), list(np), t, t_tot, t_count=0, p_count, i,j, ipar, rand_for_cell
		...
		!write (fname, "(A14,I0.6)") "../Codes/initial_stats"                             
    		!open (unit=out_unit,file='./data/mbs_E_fluc.dat',action="write",status="replace")
    		!open (unit=out_unit1,file='./data/mbs_E_av.dat',action="write",status="replace")
